...
class TestForwardWorkflowRegression(unittest.TestCase):
    ...
    def test_scenario_a_and_b_forward_progression(self):
        """
        TEST A & TEST B:
        Turn 1: 'My name is Ali and I need an appointment tomorrow.' -> name=Ali, date=tomorrow, service missing -> service_selection
        Turn 2: 'I don't know what problem I have. I just need a dental checkup.' -> service=Consultation, doctor missing -> doctor_selection (NOT service_selection)
        """
        ...
        self.assertEqual(conv_1.awaiting_input, "service_choice")
        self.assertEqual(res1.get("ui_action", {}).get("type"), "service_selection")
        ...
        self.assertIsNone(conv_2.selected_doctor_id)  # Must NOT auto-assign doctor
        self.assertEqual(conv_2.requested_date, expected_tomorrow)  # Date preserved
        self.assertEqual(conv_2.awaiting_input, "doctor_choice")
        self.assertEqual(res2.get("ui_action", {}).get("type"), "doctor_selection")
        self.assertNotEqual(res2.get("ui_action", {}).get("type"), "service_selection")

    def test_scenario_urdu_script_conversation(self):
        """
        Test Urdu script conversation:
        Turn 1: 'اسلام علیکم میرا نام علی ہے اور مجھے کل کے لیے ایک اپائنٹمنٹ چاہیے' -> Name=Ali, Date=Tomorrow -> service_selection
        Turn 2: 'مجھے پروبلم کا نہیں پتا لیکن میں فیلال دانت کی چیک اپ کے لیے آ رہا ہوں' -> Service=Consultation -> doctor_selection
        """
        conv = Conversation(business_id=self.biz.id, status="AI")
        db.session.add(conv)
        db.session.commit()
        ...
        self.assertIsNone(conv_2.selected_doctor_id)
        self.assertEqual(conv_2.requested_date, expected_tomorrow)
        self.assertEqual(conv_2.awaiting_input, "doctor_choice")
        self.assertEqual(res2.get("ui_action", {}).get("type"), "doctor_selection")
    ...
    def test_scenario_c_consultation_with_doctor(self):
        ...
        Expected: service = Dental Checkup & Consultation, doctor = Dr Sara, ui_action = date_selection
        """
        conv = Conversation(business_id=self.biz.id, status="AI")
        db.session.add(conv)
        db.session.commit()
    ...
    def test_scenario_f_repeating_service_text_preserves_state(self):
        ...
        self.assertEqual(conv_db.awaiting_input, "doctor_choice")
        self.assertEqual(res.get("ui_action", {}).get("type"), "doctor_selection")
        self.assertNotEqual(res.get("ui_action", {}).get("type"), "service_selection")
